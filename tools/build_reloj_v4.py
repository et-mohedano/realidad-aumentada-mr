...
OUT = Path('/mnt/data/mr_ar_logo_variantes_v4')
ASSETS = OUT / 'assets'
...
BASE=rgba('#b99e84'); GREEN=rgba('#5b6f38'); GOLD=rgba('#b98739')
...
def clock_face(side):
    ...
    for length,thick,ang in [(0.31,0.035,math.radians(4)),(0.23,0.027,math.radians(-58))]:
        if side in ('+z','-z'):
            ...
            h.apply_translation(basis(0,length*0.18,0.012))
...
# ---------- cupola and more complete diamond-like roof ----------
box((2.28,0.08,2.28),(0,7.27,0),TRIM,'cupola_base')
for x in (-0.92,0.92):
    for z in (-0.92,0.92):
        box((0.055,0.72,0.055),(x,7.68,z),RAIL)
for z in (-0.99,0.99): box((1.85,0.045,0.04),(0,7.48,z),TRIM)
for x in (-0.99,0.99): box((0.04,0.045,1.85),(x,7.48,0),TRIM)
# eave skirt under roof
roof_skirt = trimesh.creation.cone(radius=1.52, height=0.20, sections=8)
roof_skirt.apply_translation((0,7.88,0))
add(roof_skirt, 'roof_skirt', TRIM)
# fuller roof with 8 sections to avoid flat-looking side
roof=trimesh.creation.cone(radius=1.38,height=0.78,sections=8)
roof.apply_translation((0,8.28,0)); add(roof,'roof',ROOF)
# small diamond cap to complete the top shape
cap_points = np.array([[0,0.32,0],[0.22,0,0],[0,0,0.22],[-0.22,0,0],[0,0,-0.22],[0,-0.22,0]], dtype=float)
upper_cap = trimesh.convex.convex_hull(cap_points)
upper_cap.apply_translation((0,8.70,0))
add(upper_cap,'diamond_cap',ROOF)
...
if logo_path.exists():
    ...
    for k,g in logo_scene.geometry.items():
        gg=g.copy(); gg.apply_translation(-center); gg.apply_scale(scale); gg.apply_translation((0,4.27,1.13))
        scene.add_geometry(gg,node_name=f'embedded_logo_{k}')
        logo_geoms.append(gg)

# ---------- bitmap text, moved higher ----------
FONT5 = {
...
add_bitmap_text('2DO INFORME', 3.35, 10.80, 0.22, GOLD, depth=0.11, name='txt_informe')
add_bitmap_text('2026', 1.55, 10.05, 0.24, TRIM, depth=0.12, name='txt_2026')
...
json_bytes = export['model.gltf']
gltf = json.loads(json_bytes.decode('utf-8'))
# combine existing buffers into one blob
combined_bin = bytearray()
offsets = []
for i,b in enumerate(gltf['buffers']):
    key = b['uri']
    blob = export[key]
    # 4-byte align each buffer start
    while len(combined_bin) % 4: combined_bin += b'\x00'
    offsets.append(len(combined_bin))
    combined_bin += blob
# patch existing bufferViews to buffer 0 and absolute offset
for bv in gltf['bufferViews']:
    bi = bv.get('buffer',0)
    bv['byteOffset'] = offsets[bi] + bv.get('byteOffset',0)
    bv['buffer'] = 0
# replace buffers array with single buffer placeholder
# append animation time/rotation data
while len(combined_bin) % 4: combined_bin += b'\x00'
time_offset = len(combined_bin)
# 24-second slow rotation loop
TIMES = np.array([0.,6.,12.,18.,24.], dtype=np.float32)
combined_bin += TIMES.tobytes()
while len(combined_bin) % 4: combined_bin += b'\x00'
...
# add bufferViews and accessors
if 'bufferViews' not in gltf: gltf['bufferViews'] = []
if 'accessors' not in gltf: gltf['accessors'] = []
idx_bv_time = len(gltf['bufferViews'])
gltf['bufferViews'].append({'buffer':0,'byteOffset':time_offset,'byteLength':TIMES.nbytes})
idx_bv_rot = len(gltf['bufferViews'])
gltf['bufferViews'].append({'buffer':0,'byteOffset':rot_offset,'byteLength':ROTS.nbytes})
idx_acc_time = len(gltf['accessors'])
gltf['accessors'].append({'bufferView':idx_bv_time,'componentType':5126,'count':len(TIMES),'type':'SCALAR','min':[float(TIMES.min())],'max':[float(TIMES.max())]})
idx_acc_rot = len(gltf['accessors'])
gltf['accessors'].append({'bufferView':idx_bv_rot,'componentType':5126,'count':len(ROTS),'type':'VEC4'})
# add animation on root node 0
anim = {
    'name':'GiroLentoAR',
    'samplers':[{'input': idx_acc_time, 'output': idx_acc_rot, 'interpolation':'LINEAR'}],
    'channels':[{'sampler':0, 'target':{'node':0, 'path':'rotation'}}]
}
gltf['animations'] = [anim]
# remove uris and set single buffer
for b in gltf.get('buffers',[]): b.pop('uri', None)
gltf['buffers'] = [{'byteLength': len(combined_bin)}]
# also set scene extras? not needed
# pack GLB
...
GLB_VERSION = 2
total_length = 12 + 8 + len(json_data) + 8 + len(combined_bin)
out_path = ASSETS / 'reloj_monumento_ar_v3.glb'
with open(out_path, 'wb') as f:
    f.write(struct.pack('<III', GLB_MAGIC, GLB_VERSION, total_length))
    f.write(struct.pack('<I4s', len(json_data), b'JSON'))
    f.write(json_data)
    f.write(struct.pack('<I4s', len(combined_bin), b'BIN\x00'))
    f.write(combined_bin)
print('wrote', out_path, out_path.stat().st_size)
print('animations', gltf.get('animations'))
