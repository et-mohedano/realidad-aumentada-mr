...
OUT = Path('/mnt/data/mr_ar_logo_variantes_v5')
ASSETS = OUT / 'assets'
...
if logo_path.exists():
    ...
    scale=0.82/max(logo_scene.extents[0],1e-6); center=logo_scene.bounds.mean(axis=0)
    for k,g in logo_scene.geometry.items():
        ...
        scene.add_geometry(gg,node_name=f'embedded_logo_{k}'); logo_geoms.append(gg)
...
while len(combined_bin)%4: combined_bin += b'\x00'
out_path=ASSETS/'reloj_monumento_ar_v5.glb'
